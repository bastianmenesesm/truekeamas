...
export default function CartModal() {
  ...
  if (!savedProducts.length) return <div className="nb">Guarda publicaciones con 🤍 para verlas aquí.</div>;
  return (
    <div className="ml">
      {savedProducts.map(p => (
        <div key={p.id} className="mk">
          <div className="mke">{p.photos?.[0] ? <img src={p.photos[0]} alt={p.title} /> : (p.emoji || '📦')}</div>
          <div className="mki"><div className="mkt">{p.title}</div><div className="mks">por {p.owner} · {p.location || ''}</div></div>
