...
export default function PublishModal() {
  ...
  async function handlePhotoChange(e) {
    ...
    for (const f of files) {
      if (!f.type.startsWith('image/')) { showToast('Solo se permiten imágenes.'); continue; }
  ...
  async function handleSubmit(e) {
    ...
    setProgress(10);
    try {
      const inputs = e.target.querySelector('#pInput');
      const files = inputs?.files ? Array.from(inputs.files).slice(0, 2) : [];
      const uploadedUrls = [];
  ...
  return (
    <form onSubmit={handleSubmit} className="fg">
      ...
      <div className="fl">
        ...
        <div className="pua">
          <input type="file" id="pInput" accept="image/*" multiple onChange={handlePhotoChange} />
          <div style={{ fontSize: 32, marginBottom: 8 }}>📷</div>
          <p><strong>Haz clic o arrastra fotos aquí</strong><br />JPG, PNG, WebP · Máx. 5MB por foto</p>
        </div>
        {photos.length > 0 && (
          <div className="pp2">
            {photos.map((src, i) => (
      ...
      <div className="cg2 fl">
        ...
        <label className="cr"><input type="checkbox" name="buy" /><div className="ct"><strong>Permitir compra directa</strong><span>El precio referencial queda visible.</span></div></label>
        <label className="cr"><input type="checkbox" name="mixed" defaultChecked /><div className="ct"><strong>Permitir acuerdo mixto</strong><span>Producto más dinero u otro beneficio.</span></div></label>
      </div>

      <div className="nb fl">Truekeamas muestra tu publicación como vitrina. Estado, pago y entrega se acuerdan entre usuarios.</div>
      <div className="ma fl">
