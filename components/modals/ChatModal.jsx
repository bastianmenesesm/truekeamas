...
export default function ChatModal({ mid, prod }) {
  ...
  function handleImgChange(e) {
    ...
    if (!f) return;
    if (!f.type.startsWith('image/')) { showToast('Solo imágenes.'); return; }
    if (f.size > 5 * 1024 * 1024) { showToast('Imagen supera 5MB.'); return; }
    setPendImg(f);
    setPendImgUrl(URL.createObjectURL(f));
  }
  ...
  const isTrustedUrl = (url) => url && (url.startsWith('https://res.cloudinary.com') || url.startsWith('https://firebasestorage.googleapis.com'));

  return (
    <>
      <div className="cmsg" ref={msgsRef}>
        {loading && <div className="cem"><div className="sp" style={{ margin: '0 auto 12px' }} />Cargando mensajes...</div>}
        {!loading && messages.length === 0 && (
          <div className="cem">✉️ Sé el primero en escribir.<br /><small style={{ color: 'var(--mu)' }}>Deja claro qué ofreces y qué esperas.</small></div>
        )}
        {messages.map(m => {
          const me = m.senderId === currentUser?.uid;
          const ts = m.createdAt?.toDate ? m.createdAt.toDate() : new Date();
          const t = ts.toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' });
          return (
            <div key={m.id} className={`cb ${me ? 'me' : 'them'}`}>
              {!me && <div className="cbn">{m.senderName || 'Usuario'}</div>}
              {isTrustedUrl(m.imageUrl) && <img className="cbi" src={m.imageUrl} alt="" onClick={() => window.open(m.imageUrl, '_blank')} loading="lazy" />}
      ...
      <div className="cf">
        ...
        <div className="cir">
          ...
          <input ref={inputRef} placeholder="Escribe tu mensaje..." autoComplete="off" onKeyDown={handleKeyDown} />
